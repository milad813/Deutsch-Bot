...
from ui import (_bold_word_in_sentence, _short_label, back_inline_keyboard,
                esc, progress_bar, render)
...
def _ltr_progress_header(context) -> str:
    ...
    pos = context.user_data.get(
        "ltr_current_word_pos", context.user_data.get("ltr_main_index", 1)
    )
...
async def start_study_session(query, context, lesson_id: int):
    ...
    new_words = db.get_new_word_objects(
        user_id=user_id, lesson_id=lesson_id, limit=remaining
    )
    ...
    if not all_words:
        await render(
            query,
            "🎉 همه کلمات این درس را یاد گرفته‌ای!",
            reply_markup=back_inline_keyboard(),
        )
        return
...
async def handle_ltr_start(query, context, suffix=None):
    if context.user_data.get("ltr_round", 1) == 2 and not context.user_data.get(
        "ltr_round2_started"
    ):
        context.user_data["ltr_word_results"] = {}
    ...
    if word.word_type == "Noun" and article in ("der", "die", "das"):
        ...
        if word.example_de:
            ...
            cloze = QuizService.create_cloze_quiz(
                word.german, word.persian, word.example_de
            )
...
async def handle_ltr_answer(query, context, suffix: str):
    ...
    if not word_id or state not in {
        "test_immediate",
        "test_delayed_1",
        "test_delayed_2",
    }:
...
async def _advance_ltr(query, context):
    ...
    if main_index < len(word_ids):
        ...
        context.user_data["ltr_main_progress"] = (
            context.user_data.get("ltr_main_progress", 0) + 1
        )
        context.user_data["ltr_current_word_id"] = word_id
        context.user_data["ltr_current_word_pos"] = context.user_data[
            "ltr_main_progress"
        ]
        context.user_data["ltr_state"] = "intro"
        context.user_data["ltr_retry_stage"] = None
    ...
    if lesson_id:
        kb_rows.extend(
            [
                [
                    InlineKeyboardButton(
                        "📚 ۱۰ کلمه بعدی", callback_data=f"study_lesson:{lesson_id}"
                    )
                ],
                [
                    InlineKeyboardButton(
                        "🎴 فلش‌کارت این درس",
                        callback_data=f"flashcard_lesson:{lesson_id}",
                    )
                ],
                [
                    InlineKeyboardButton(
                        "🤖 کوییز این درس",
                        callback_data=f"quiz_from_lesson:{lesson_id}",
                    )
                ],
            ]
        )
