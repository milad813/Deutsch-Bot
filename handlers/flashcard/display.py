...
from constants import (BTN_AGAIN, BTN_BACK, BTN_EASY, BTN_GOOD, BTN_HARD,
                       BTN_SHOW_MEANING, BTN_SKIP, BTN_TTS)
            ...
            [
                ...
                [
                    InlineKeyboardButton(
                        BTN_SHOW_MEANING, callback_data=f"flip_card:{word.id}"
                    )
                ],
                [
                    InlineKeyboardButton(
                        BTN_SKIP, callback_data=f"skip_flashcard:{word.id}"
                    )
                ],
    ...
    def create_back_keyboard(word: Word) -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton(
                        BTN_AGAIN, callback_data=f"rate_card:{word.id}:1"
                    )
                ],
                [
                    InlineKeyboardButton(
                        BTN_HARD, callback_data=f"rate_card:{word.id}:2"
                    )
                ],
                [
                    InlineKeyboardButton(
                        BTN_GOOD, callback_data=f"rate_card:{word.id}:3"
                    )
                ],
                [
                    InlineKeyboardButton(
                        BTN_EASY, callback_data=f"rate_card:{word.id}:4"
                    )
                ],
            ]
        )
